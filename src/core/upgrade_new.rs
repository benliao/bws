...
impl UpgradeManager {
    ...
    pub async fn initiate_upgrade(&self, binary_path: Option<String>) -> Result<(), UpgradeError> {
        #[cfg(not(unix))]
        {
            warn!("Zero-downtime upgrades are not fully supported on this platform");
            return Err(UpgradeError::UnsupportedPlatform);
        }
        ...
        #[cfg(unix)]
        {
            ...
            if *upgrade_in_progress {
                return Err(UpgradeError::ProcessSpawn("Upgrade already in progress".to_string()));
            }
    ...
    fn spawn_new_process(&self, binary_path: &str, upgrade_file: &str) -> Result<Child, UpgradeError> {
    ...
    /// Handle upgrade from a previous process
    pub async fn handle_upgrade_from_file(&self, upgrade_file: &str) -> Result<(), UpgradeError> {
        info!("Processing upgrade from file: {}", upgrade_file);

        let upgrade_data = self.read_upgrade_data(upgrade_file)?;

        // Update configuration
        *self.config.write().await = upgrade_data.config;

        // Restore server state
        *self.state.write().await = upgrade_data.server_state;

        // Clean up upgrade file
        if let Err(e) = std::fs::remove_file(upgrade_file) {
            warn!("Failed to clean up upgrade file: {}", e);
        }

        info!("Upgrade from previous process completed");
        Ok(())
    }
